...
/datum/species/jelly/proc/Cannibalize_Body(mob/living/carbon/human/H)
	...
	to_chat(H, span_danger("My [consumed_limb] is drawn back into my body, unable to maintain its shape!"))
	qdel(consumed_limb)
	H.blood_volume += 20
...
/datum/action/innate/regenerate_limbs/Activate()
	...
	if(limbs_to_heal.len < 1)
		...
		return
	to_chat(H, span_notice("I focus intently on my missing [limbs_to_heal.len >= 2 ? "limbs" : "limb"]..."))
...
/datum/action/innate/integrate_extract/Activate()
	...
	if(species.current_extract)
		...
		if(istype(I, /obj/item/slime_extract))
			...
			to_chat(H, span_warning("I need to hold an unused slime extract in my active hand!"))
